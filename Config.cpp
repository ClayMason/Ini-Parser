...
Config::Config (const char* fname) {
  ...
  // otherwise, read file
  if (!isValid (std::string(fname))) throw "Invalid filetype. (Expected .ini file)\n";
  std::ifstream ini_file;
  ini_file.open(fname);

  if ( !ini_file ) throw std::string("Could not open file ").append(fname).c_str();
...
QuantumProp* Config::getValueType (std::string value){
  ...
  else if ( std::regex_match(value.c_str(), std::regex("^(Yes|No)$"))) {
    // printf ("returning bool value type\n");
    bool bool_val;
    if (value.compare("Yes") == 0) bool_val = true;
    else bool_val = false;
  ...
  else if ( std::regex_match(value.c_str(), std::regex("^([0-9]+)?(\\.){1}([0-9]+)?$"))) {
  ...
  else if ( std::regex_match(value.c_str(), std::regex("^([A-Z]{1}(:))?(()/)?[a-zA-Z_-]+){1}(((/){1}[a-zA-Z_-]+)+)?$"))) {
...
ConfSection* Config::getSection (const char* section_name) {
  ...
  if (q_itr == ini_data.end()) return 0;
  else return &(q_itr->second);
}
...
bool Config::addEntry(char* section, std::pair<const char*, QuantumProp*> entry) {
  ...
  if (CHECK) printf("Section %s does not exist. Creating [%s]\n", section, section);
  else printf("Section %s already exists. Adding to existing...\n", section);
  /// END DEBUGGING ///

  if (q_itr == ini_data.end()) { // if the iterator could not find the section, create the section and add it
  ...
  else delete section; // section exists already so delete the new variable created in 1st if.
...
bool ConfSection::addEntry(std::pair<const char*, QuantumProp*> new_entry) {
  ...
  if (new_entry.second->strValue()==0) return false;
  else {
    ...
    if (result.second) printf("Entry added ... size = %d\n", this->size());
    else printf ("Error adding %s to %s ... \n", new_entry.first, this->section_name);
...
bool ConfSection::updateEntry(std::pair<const char*, QuantumProp*> updated_entry) {
  ...
  if (s_itr == sect_map.end()) return false;
  else s_itr->second = updated_entry.second;
